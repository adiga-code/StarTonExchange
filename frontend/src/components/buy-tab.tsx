...
export default function BuyTab({ user, onShowLoading, onHideLoading }: BuyTabProps) {
  ...
  // Получаем публичные настройки для отображения
  ...
  // Хардкод для быстрых покупок (остается)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Rest of the component stays the same, but price calculation display changes */}
      <motion.div
        className="bg-white dark:bg-[#1A1A1C] rounded-xl p-4 shadow-lg dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] border border-gray-200 dark:border-white/10"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <div className="space-y-4">
          ...
          {/* Price Display - разное отображение для stars и ton */}
          {priceCalculation && (
            <motion.div
            ...
            >
              {selectedCurrency === 'stars' ? (
                // Для звезд: стоимость, экономия, итого
                <>
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-gray-600 dark:text-gray-400">Экономия:</span>
                    <span className="font-semibold text-green-500">{priceCalculation.savings_percentage}%</span>
                  </div>
      ...
      {/* Quick Buy Options - убираем хардкод расчета цен */}
      <motion.div
        className="bg-white dark:bg-[#1A1A1C] rounded-xl p-4 shadow-lg dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] border border-gray-200 dark:border-white/10"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
