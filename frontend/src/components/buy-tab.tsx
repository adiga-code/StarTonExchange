...
export default function BuyTab({ user, onShowLoading, onHideLoading }: BuyTabProps) {
  ...
  const purchaseMutation = useMutation({
    mutationFn: async (data: { currency: string; amount: number; rub_amount: number; username?: string }) => {
  ...
  const prices = {
    stars: adminSettings?.stars_price ? parseFloat(adminSettings.stars_price) : 1.50,
  ...
  return (
    <div className="space-y-4">
          ...
          <div>
            ...
            {userPhoto ? (
              <div className="flex items-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                <img src={userPhoto.photo_url} alt="User" className="w-10 h-10 rounded-full mr-3" />
                <div>
                  <p className="font-medium text-green-800 dark:text-green-200">{userPhoto.first_name}</p>
                  <p className="text-sm text-green-600 dark:text-green-400">@{recipientUsername}</p>
                </div>
              </div>
            ) : (
              <div className="space-y-2">
                <div className="relative">
                  <Input
                    ...
                    className={`mt-1 pl-8 bg-white dark:bg-[#1A1A1C] border-gray-200 dark:border-white/20 ${userError ? 'border-red-500' : ''}`}
                    disabled={isProcessing || isLoadingPhoto}
                  />
                  <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400">@</span>
                  {isLoadingPhoto && (
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                      <div className="animate-spin w-4 h-4 border-2 border-[#4E7FFF] border-t-transparent rounded-full"></div>
                    </div>
                  )}
                </div>
                {userError && (
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                Купить за ₽{priceCalculation?.base_price ? parseFloat(priceCalculation.base_price).toLocaleString() : '0'}
              </>
            )}
          </Button>
        </div>
      ...
      <PaymentMethodModal
        ...
        onSBPPayment={handleSBPPayment}
        amount={priceCalculation?.base_price || '0'}
        currency={selectedCurrency}
      />
    </div>
  );
