...
export default function BuyTab({ user, onShowLoading, onHideLoading }: BuyTabProps) {
  ...
  // Purchase mutation for FreeKassa
  const purchaseMutation = useMutation({
    ...
    onSuccess: (paymentData) => {
      ...
      if (window.Telegram?.WebApp) {
        // В Telegram используем openLink для внешних ссылок
      ...
      toast({
        ...
        description: "Откройте страницу оплаты FreeKassa",
      });
    },
    onError: (error: any) => {
  ...
  // Direct payment handler - opens FreeKassa immediately
