...
import { Star, Bitcoin, Calculator, ExternalLink } from "lucide-react";
...
export default function BuyTab({ user, onShowLoading, onHideLoading }: BuyTabProps) {
  ...
  const handlePurchase = async () => {
    if (!isValidAmount || !priceCalculation || isProcessing) return;
  ...
  return (
    <div className="space-y-4">
      {/* Currency Selector */}
      <motion.div
        className="bg-white dark:bg-[#1A1A1C] rounded-xl p-4 shadow-lg border dark:border-white/10"
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            <Label htmlFor="amount" className="text-gray-600 dark:text-gray-400">Количество</Label>
            <div className="relative mt-2">
              <Input
                ...
                className="pr-12 bg-gray-50 dark:bg-[#0E0E10] border-gray-200 dark:border-white/20 focus:border-[#4E7FFF]"
                disabled={isProcessing}
              />
          ...
          <Button
            ...
            disabled={!isValidAmount || purchaseMutation.isPending || isProcessing}
        ...
        <div className="grid grid-cols-2 gap-3">
          {quickBuyOptions.map((optionAmount) => (
            <motion.button
              ...
              className="p-3 rounded-lg bg-gray-50 dark:bg-[#0E0E10] hover:bg-[#4E7FFF]/20 border border-gray-200 dark:border-white/10 hover:border-[#4E7FFF]"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              disabled={isProcessing}
            >
      ...
      >
        <h4 className="font-semibold mb-2 flex items-center">🔒 Безопасная оплата</h4>
